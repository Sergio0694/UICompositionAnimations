...
namespace UICompositionAnimationsLegacy.Animations
{
    ...
    internal sealed class CompositionAnimationBuilder : AnimationBuilderBase<Action<TimeSpan>>
    {
        ...
        /// <inheritdoc/>
        protected override IAnimationBuilder OnOpacity(double? from, double to, EasingFunctionNames ease) => OnScalarAnimation(nameof(Visual.Opacity), from, to, ease);

        /// <inheritdoc/>
        protected override IAnimationBuilder OnTranslation(TranslationAxis axis, double? from, double to, EasingFunctionNames ease) => OnScalarAnimation($"Translation.{axis}", from, to, ease);

        /// <inheritdoc/>
        protected override IAnimationBuilder OnTranslation(Vector2? from, Vector2 to, EasingFunctionNames ease = EasingFunctionNames.Linear) => OnVector3Animation("Translation", from, to, ease);

        /// <inheritdoc/>
        protected override IAnimationBuilder OnOffset(TranslationAxis axis, double? from, double to, EasingFunctionNames ease) => OnScalarAnimation($"{nameof(Visual.Offset)}.{axis}", from, to, ease);

        /// <inheritdoc/>
        protected override IAnimationBuilder OnOffset(Vector2? from, Vector2 to, EasingFunctionNames ease = EasingFunctionNames.Linear) => OnVector3Animation(nameof(Visual.Offset), from, to, ease);
        ...
        /// <inheritdoc/>
        protected override IAnimationBuilder OnSize(TranslationAxis axis, double? from, double to, EasingFunctionNames ease) => OnScalarAnimation($"{nameof(Visual.Size)}.{axis}", from, to, ease);

        /// <inheritdoc/>
        protected override IAnimationBuilder OnSize(Vector2? from, Vector2 to, EasingFunctionNames ease = EasingFunctionNames.Linear) => OnVector3Animation(nameof(Visual.Size), from, to, ease);
        ...
        private IAnimationBuilder OnScalarAnimation(string property, double? from, double to, EasingFunctionNames ease)
        {
            AnimationFactories.Add(duration =>
            {
                // Stop the animation and get the easing function
        ...
        [MustUseReturnValue, NotNull]
        private IAnimationBuilder OnVector3Animation(string property, Vector2? from, Vector2 to, EasingFunctionNames ease)
        {
            AnimationFactories.Add(duration =>
            {
                // Stop the animation and get the easing function
