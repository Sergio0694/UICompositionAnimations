...
namespace FluentExtensions.UI.Helpers
{
    ...
    public static class DispatcherHelper
    {
        ...
        public static void Run(Action callback) => CoreDispatcher.Run(callback);
        ...
        public static void Run(Func<Task> asyncCallback) => CoreDispatcher.Run(asyncCallback);

        /// <summary>
        /// Executes a given <see cref="Action"/> on the UI thread and waits for it to be completed
        /// </summary>
        /// <param name="callback">The <see cref="Action"/> to execute on the UI thread</param>
        public static Task RunAsync(Action callback) => CoreDispatcher.RunAsync(callback);

        /// <summary>
        /// Executes a given <see cref="Func{TResult}"/> that returns a <see cref="Task"/> on the UI thread and waits for it to be completed
        /// </summary>
        /// <param name="asyncCallback">The <see cref="Func{TResult}"/> to execute on the UI thread</param>
        public static Task RunAsync(Func<Task> asyncCallback) => CoreDispatcher.RunAsync(asyncCallback);
        ...
        public static Task<T> GetAsync<T>(Func<T> function) => CoreDispatcher.GetAsync(function);
        ...
        public static Task<T> GetAsync<T>(Func<Task<T>> function) => CoreDispatcher.GetAsync(function);
    }
}
